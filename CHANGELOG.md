...
- Colors and fonts are now authored once, in `packages/shared/src/tokens/` (Style Dictionary format), and generated into the web app's CSS (`app/theme-tokens.generated.css`) and the mobile app's `theme.ts` from that single source — closing a gap where the two had to be kept in sync by hand.
