...
afterEach(() => {
  try {
    storage.setItem(`chatbot-history-${baseBot.name}`, JSON.stringify([]));
  } catch {}
  try {
    storage.setItem(`lastPlayedAudioHash-${baseBot.name}`, "");
  } catch {}
});
...
describe("useChatController additional small branches", () => {
  ...
  it("blur resets CSS pad and classes for visualViewport fallback", async () => {
    ...
    await waitFor(() =>
      expect(
        parseInt(root.style.getPropertyValue("--vv-keyboard-pad") || "0", 10),
  ...
  it("handleHeaderLinkClick focuses the input when present", async () => {
    ...
    act(() => {
      result.current.handleHeaderLinkClick();
    });
  ...
  it("handleScroll schedules a single RAF when called rapidly", async () => {
    ...
    chatEl.scrollTop = 0;
    act(() => {
      result.current.chatBoxRef.current = chatEl;
    });
