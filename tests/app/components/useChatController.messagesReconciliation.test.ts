...
jest.mock("../../../src/utils/storage", () => ({
  ...
  setItem: jest.fn((key: string, value: string) => {
    storedHistory[key] = value;
  }),
  removeItem: jest.fn((key: string) => {
    delete storedHistory[key];
  }),
...
describe("useChatController — server history reconciliation", () => {
  ...
  it("never fetches /api/messages for a guest", async () => {
    ...
    await waitFor(() => {
      expect(
        mockAuthenticatedFetch.mock.calls.some((c) => String(c[0]).startsWith("/api/messages")),
      ).toBe(false);
    });
  });
  ...
  it("does not fire a duplicate intro request when server history exists but local storage is empty (new device / cleared cache)", async () => {
    ...
    expect(
      mockAuthenticatedFetch.mock.calls.some((c) => String(c[0]).startsWith("/api/chat")),
    ).toBe(false);
  });
