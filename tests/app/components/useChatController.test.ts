...
describe("useChatController additional branches (merged)", () => {
  ...
  it("retrieves voiceConfig from saved bot in storage", async () => {
    const savedBot = {
      ...baseBot,
      voiceConfig: { name: "en-US-Wavenet-B", languageCodes: ["en-US"], ssmlGender: 1 },
    };
    mockLoadVoiceConfig.mockReturnValue(null);
    mockApiGetVoiceConfigForCharacter.mockRejectedValue(new Error("should not fetch"));
    mockStorage.getItem.mockImplementation((key: string) =>
      key === "chatbot-bot" ? JSON.stringify(savedBot) : null,
    );
  ...
  it("handles invalid JSON for saved history gracefully", async () => {
    ...
    mockStorage.getItem.mockImplementation((key: string) =>
      key === chatHistoryKey ? "invalid{" : null,
    );
  ...
  it("visualViewport focus/blur sets CSS pad and classes", async () => {
    ...
    const chatHistoryKey = `chatbot-history-${baseBot.name}`;
    mockStorage.getItem.mockImplementation((key: string) =>
      key === chatHistoryKey ? JSON.stringify([{ sender: "x", text: "y" }]) : null,
    );
  ...
  it("registers window resize listener when visualViewport not available and cancels RAF on cleanup", () => {
    ...
    window.addEventListener = ((ev: string, cb: (...args: unknown[]) => void) => {
      if (ev === "resize") savedResize = cb;
    }) as unknown as typeof window.addEventListener;
    window.removeEventListener = jest.fn();

    const rafId = 123;
    // Do not invoke the callback synchronously so the RAF id remains pending until unmount
    window.requestAnimationFrame = ((_: FrameRequestCallback) => {
      return rafId;
    ...
    mockAuthenticatedFetch.mockImplementation((url: string) => {
      ...
      if (url === "/api/chat")
        return Promise.resolve(
          mockResponse({ reply: "Audio reply", audioFileUrl: "/audio/error.mp3" }),
        );
      return Promise.resolve(mockResponse({}));
    });
  ...
  it("logs info when audio playback is aborted (AbortError)", async () => {
    mockAuthenticatedFetch.mockImplementation((url: string) => {
      ...
      if (url === "/api/chat")
        return Promise.resolve(
          mockResponse({ reply: "Audio reply", audioFileUrl: "/audio/abort.mp3" }),
        );
      return Promise.resolve(mockResponse({}));
    });
  ...
  it("sendMessage: production logging suppressed for retry info", async () => {
    ...
    try {
      ...
      const foundStart = mockLogEvent.mock.calls.some(
        (call) => call[1] === "chat_send_retry_start",
      );
      const foundSuccess = mockLogEvent.mock.calls.some(
        (call) => call[1] === "chat_send_retry_success",
      );
  ...
  it("handleDownloadTranscript logs error and shows alert when download fails", async () => {
    ...
    const { result } = renderHook(() => useChatController(baseBot));
    await act(async () => {
      await result.current.handleDownloadTranscript();
    });
  ...
  it("handleAudioToggle toggles audio, persists preference, and focuses input", async () => {
    ...
    act(() => {
      ...
      (result.current as unknown as { audioRef: React.RefObject<HTMLAudioElement> }).audioRef =
        mockAudioRef;
    });

    // Initially audioEnabled is true; toggling should persist false
    act(() => {
      result.current.handleAudioToggle();
    });
  ...
  it("logMessage returns early when session is missing (no /api/log-message call)", async () => {
    ...
    mockAuthenticatedFetch.mockImplementation((url: string) => {
      if (url === "/api/health") return Promise.resolve(mockResponse({}));
      if (url === "/api/chat")
        return Promise.resolve(mockResponse({ reply: "Ok", audioFileUrl: null }));
  ...
  it("intro generation failure sets introError and logs", async () => {
    ...
    expect(mockLogEvent).toHaveBeenCalledWith(
      ...
      "Failed to generate intro or voice config. Please recreate the bot.",
      expect.any(Object),
    );
  });
  ...
  it("sendMessage returns early when API health is false", async () => {
    ...
    mockAuthenticatedFetch.mockImplementation((url: string) => {
      if (url === "/api/health") return Promise.reject(new Error("down"));
      if (url === "/api/chat")
        return Promise.resolve(mockResponse({ reply: "Should not be called" }));
      return Promise.resolve(mockResponse({}));
    });
    ...
    const initialChatCalls = mockAuthenticatedFetch.mock.calls.filter(
      (c) => c[0] === "/api/chat",
    ).length;
    ...
    const chatCallsAfter = mockAuthenticatedFetch.mock.calls.filter(
      (c) => c[0] === "/api/chat",
    ).length;
    ...
    mockAuthenticatedFetch.mockImplementation((url: string) => {
      ...
      if (url === "/api/chat")
        return new Promise((res) =>
          setTimeout(() => res(mockResponse({ reply: "reply", audioFileUrl: null })), 30),
        );
      return Promise.resolve(mockResponse({}));
    });
  ...
  it("successful audio playback stores last played hash", async () => {
    mockAuthenticatedFetch.mockImplementation((url: string) => {
      ...
      if (url === "/api/chat")
        return Promise.resolve(
          mockResponse({ reply: "Audio reply", audioFileUrl: "/audio/success.mp3" }),
        );
      return Promise.resolve(mockResponse({}));
    });
    ...
    expect(s2.setItem).toHaveBeenCalledWith(
      expect.stringContaining("lastPlayedAudioHash-"),
      expect.any(String),
    );
  });
  ...
  it("sendMessage sets error and logs when voiceConfig is missing", async () => {
    ...
    const errLogFound = mockLogEvent.mock.calls.some(
      (c) =>
        c[0] === "error" &&
        [
  ...
  it("ensureVoiceConfig falls back to api_getVoiceConfigForCharacter when load/storage throw", async () => {
    ...
    mockLoadVoiceConfig.mockImplementation(() => {
    ...
    const fetchedConfig = {
      ...baseBot.voiceConfig,
      name: "fetched-voice",
    } as unknown as import("../../../src/utils/characterVoices").CharacterVoiceConfig;
  ...
  it("handleAudioToggle tolerates storage.setItem throwing", async () => {
    ...
    (storage as jest.Mocked<typeof storage>).setItem.mockImplementation(() => {
  ...
  it("profileApiCall logs timing for Log Message operation", async () => {
    ...
    mockAuthenticatedFetch.mockImplementation((url: string, _opts?: unknown) => {
      if (url === "/api/health") return Promise.resolve(mockResponse({}));
      if (url === "/api/chat")
        return Promise.resolve(mockResponse({ reply: "Hello", audioFileUrl: null }));
    ...
    const found = mockLogEvent.mock.calls.some(
      (call) =>
        call[1] === "chat_api_timing" &&
        call[3] &&
        (call[3] as Record<string, unknown>).operation === "Log Message",
    );
    expect(found).toBe(true);
  });
  ...
  it("plays audio and stores lastPlayedAudioHash on success", async () => {
    mockAuthenticatedFetch.mockImplementation((url: string, _opts?: unknown) => {
      ...
      if (url === "/api/chat")
        return Promise.resolve(
          mockResponse({ reply: "Audio reply", audioFileUrl: "http://example/audio.mp3" }),
        );
      return Promise.resolve(mockResponse({}));
    });
    ...
    expect(storage.setItem).toHaveBeenCalledWith(
      `lastPlayedAudioHash-${mockBot.name}`,
      expect.any(String),
    );
  });
  ...
  it("logs info when audio playback is aborted (AbortError)", async () => {
    mockAuthenticatedFetch.mockImplementation((url: string, _opts?: unknown) => {
      ...
      if (url === "/api/chat")
        return Promise.resolve(
          mockResponse({ reply: "Audio abort", audioFileUrl: "http://example/abort.mp3" }),
        );
      return Promise.resolve(mockResponse({}));
    });
  ...
  it("logs error when audio playback fails with non-abort error", async () => {
    mockAuthenticatedFetch.mockImplementation((url: string, _opts?: unknown) => {
      ...
      if (url === "/api/chat")
        return Promise.resolve(
          mockResponse({ reply: "Audio fail", audioFileUrl: "http://example/fail.mp3" }),
        );
      return Promise.resolve(mockResponse({}));
    });
