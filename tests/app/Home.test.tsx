...
jest.mock(
  ...
  () => (importFunc: () => Promise<{ default: React.ComponentType<unknown> }>) => {
    ...
    const mockImport = importFunc as unknown as {
      (): { then: (cb: (m: { default: React.ComponentType<unknown> }) => void) => void };
    };
    try {
...
describe("Home component URL parameter functionality", () => {
  ...
  it("saves bot to storage when bot is created", async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockStorage.setVersionedJSON).toHaveBeenCalledWith(
        `voiceConfig-${newBot.name}`,
        newBot.voiceConfig,
        1,
      );
      expect(screen.getByTestId("chat-page")).toBeInTheDocument();
  ...
  it("saves bot to storage with voiceConfig when loaded from storage", async () => {
    ...
    await waitFor(() => {
      expect(mockStorage.setVersionedJSON).toHaveBeenCalledWith(
        `voiceConfig-${existingBot.name}`,
        existingBot.voiceConfig,
        1,
      );
    });
  });
