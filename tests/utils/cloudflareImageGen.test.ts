...
describe('generateImageWithCloudflare', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        process.env = { ...OLD_ENV, CLOUDFLARE_ACCOUNT_ID: 'acct-123', CLOUDFLARE_API_TOKEN: 'token-abc' };
    });
    ...
    it('returns null and logs when CLOUDFLARE_ACCOUNT_ID is not configured', async () => {
        ...
        expect(mockLogEvent).toHaveBeenCalledWith('info', 'avatar_cloudflare_not_configured', expect.any(String));
    });
    ...
    it('returns null on a non-ok HTTP response', async () => {
        ...
        const result = await generateImageWithCloudflare('a prompt');
        expect(result).toBeNull();
        expect(mockLogEvent).toHaveBeenCalledWith('warn', 'avatar_cloudflare_http_error', expect.any(String), expect.any(Object));
    });
    ...
    it('returns null when the API responds success:false', async () => {
        ...
        const result = await generateImageWithCloudflare('a prompt');
        expect(result).toBeNull();
        expect(mockLogEvent).toHaveBeenCalledWith('warn', 'avatar_cloudflare_no_image', expect.any(String), expect.any(Object));
    });
