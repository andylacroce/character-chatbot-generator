...
describe("downloadTranscript", () => {
  ...
  it("throws if window.URL.createObjectURL is not available", async () => {
    ...
    await expect(downloadTranscript([])).rejects.toThrow(
      "Browser does not support required APIs for downloading transcripts",
    );
  ...
  it("wraps non-Error response.text() rejections in message string (cond-expr branch)", async () => {
    fetchMock.mockResolvedValue({
      ok: true,
      text: () => {
        throw "text read failure string";
      },
    });
    await expect(downloadTranscript([])).rejects.toThrow(
      "Failed to read response: text read failure string",
    );
  });
