...
describe('generateImageWithPollinations', () => {
    ...
    it('returns a data URL on success, calling the Pollinations endpoint with the encoded prompt', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('image.pollinations.ai/prompt/a%20cool%20prompt'));
    ...
    it('returns null on a non-ok HTTP response', async () => {
        ...
        expect(mockLogEvent).toHaveBeenCalledWith('warn', 'avatar_pollinations_http_error', expect.any(String), expect.any(Object));
    });
    ...
    it('returns null when the response body is empty', async () => {
        ...
        const result = await generateImageWithPollinations('a prompt');
        expect(result).toBeNull();
        expect(mockLogEvent).toHaveBeenCalledWith('warn', 'avatar_pollinations_empty', expect.any(String));
    });
});
