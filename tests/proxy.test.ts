...
describe("proxy", () => {
  ...
  it("fails closed with 500 when API_SECRET is not configured", () => {
    ...
    expect(logEvent).toHaveBeenCalledWith("error", "api_secret_missing", expect.any(String), {});
  });
  ...
  describe("allowed origins", () => {
    it.each([
      ...
      [
        "a git preview deployment",
        "https://character-chatbot-generator-git-my-branch-andylacroces-projects.vercel.app",
      ],
      [
        "a hashed preview deployment",
        "https://character-chatbot-generator-abc123-andylacroces-projects.vercel.app",
      ],
    ])("allows %s without an API key", (_label, origin) => {
      const res = proxy(makeRequest({ method: "POST", headers: { origin, host: PROD_HOST } }));
  ...
  describe("look-alike origins", () => {
    it.each([
      ...
      [
        "a preview host under another scope",
        "https://character-chatbot-generator-git-main-attacker-projects.vercel.app",
      ],
      ["plain http in production", `http://${PROD_HOST}`],
      [
        "an allowed host smuggled into a path",
        "https://evil.com/https://character-chatbot-generator.vercel.app",
      ],
      [
        "an allowed host smuggled into userinfo",
        "https://character-chatbot-generator.vercel.app@evil.com",
      ],
  ...
  describe("requests with no Origin or Referer", () => {
    it("allows a safe method on a first-party host", () => {
      const res = proxy(makeRequest({ method: "GET", headers: { host: PROD_HOST } }));

      expect(allowed(res)).toBe(true);
    });

    it("allows a safe method on localhost", () => {
      const res = proxy(makeRequest({ method: "HEAD", headers: { host: "localhost:3000" } }));
    ...
    it("rejects a POST on a first-party host without an API key", () => {
      ...
      const res = proxy(makeRequest({ method: "POST", headers: { host: PROD_HOST } }));
