...
describe("log-message API", () => {
  ...
  describe("input validation", () => {
    it.each([
      [
        "a missing sender",
        { ...validBody, sender: undefined },
        "Sender, text, sessionId, and sessionDatetime required",
      ],
      [
        "an undefined text",
      ...
      [
        "a missing sessionId",
        { ...validBody, sessionId: undefined },
        "Sender, text, sessionId, and sessionDatetime required",
      ],
      [
        "a missing sessionDatetime",
        { ...validBody, sessionDatetime: undefined },
        "Sender, text, sessionId, and sessionDatetime required",
      ],
      ["an over-long sender", { ...validBody, sender: "x".repeat(101) }, "Invalid sender"],
      [
        "a non-string sender",
        { ...validBody, sender: { toString: (): string => "x" } },
        "Invalid sender",
      ],
      ...
      [
        "an over-long sessionDatetime",
        { ...validBody, sessionDatetime: "x".repeat(31) },
        "Invalid sessionDatetime",
      ],
      [
        "a non-string sessionDatetime",
        { ...validBody, sessionDatetime: 99 },
        "Invalid sessionDatetime",
      ],
  ...
  describe("local file storage", () => {
    it("appends the entry to a session-scoped log file", async () => {
      const res = makeRes();
      await handler(
        makeReq(validBody, { headers: { "x-forwarded-for": "198.51.100.4, 10.0.0.1" } }),
        res,
      );
