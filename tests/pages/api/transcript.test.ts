...
describe("transcript API", () => {
  ...
  describe("validation", () => {
    it.each([
      ...
      [
        "a bot without a string name",
        { messages, bot: { name: 1, avatarUrl: "/a.png" } },
        "bot.name must be a string",
      ],
      [
        "a bot without a string avatarUrl",
        { messages, bot: { name: "Ada", avatarUrl: 1 } },
  ...
  describe("rendering", () => {
    ...
    it("renders the bot header and avatar when a bot is supplied", async () => {
      const res = makeRes();
      await handler(
        makeReq({ messages, bot: { name: "Ada Lovelace", avatarUrl: "/avatars/ada.png" } }),
        res,
      );
